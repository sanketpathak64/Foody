...
public class userAttendance extends AppCompatActivity {
    ...
    private DatabaseReference mref,rref,eref;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mealsText=(TextView)findViewById(R.id.umeals);
        rd=findViewById(R.id.rdate);
        ed=findViewById(R.id.edate);
        ll=findViewById(R.id.uat);
        meals=mealsText.getText().toString();
        mAuth=FirebaseAuth.getInstance();
        currentUser=mAuth.getCurrentUser().getUid();
        mref= FirebaseDatabase.getInstance().getReference().child("users").child(userID).child("attendance").child(currentUser).child("attend");
        eref= FirebaseDatabase.getInstance().getReference().child("users").child(userID).child("attendance").child(currentUser).child("expiry_date");
        rref= FirebaseDatabase.getInstance().getReference().child("users").child(userID).child("attendance").child(currentUser).child("renewed_date");
